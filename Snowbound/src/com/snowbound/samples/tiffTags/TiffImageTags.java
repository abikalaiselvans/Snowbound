...
import Snow.Snowbnd;
public class TiffImageTags
{	
	/**
	 * 
	 */
	private static final long serialVersionUID = -6343133750159309477L;
    ...
    private static byte[] tagNamesBuff;
    ...
    public TiffImageTags(PrintStream log)
    {
    	super();
    	
    	//this.fileName = fileName;
    	TiffImageTags.log = log;
    	if (log != null){
			log.println("\t Dumping Tiff Tags \t Using RasterMaster ");
			log.println("------------------------------------------------------------------------");
		}
    }
    ...
    {
    	 int arrayIndex = (tag * -1) - 1;
    	 try
         {
             String name = TAG_DESCRIPTION[arrayIndex];
             return name;
         }
    	 catch (Exception e)
         {
    ...
    public static void saveDescription(String fileDescription)
    {
    	String tiffid = new String(fileDescription);
    	if(tiffid != null)
    	{
    		saveTags(getTagDescription(IMAGE_DESCRIPTION), tiffid);
    	}
    }
    
    public static byte[] stringBuff(int tagVal)
    {
    	byte[] strgByte = new byte[storeTags.length]; 
    	if(tagVal > 0)
    	{
    		strgByte = new byte[tagVal];
    	}	  	
    	return strgByte;
    }
    public static int[] returnedTagVal(int tagVal)
    {
    	...
    	int[] tagReturned = new int[tagVal];
    	return tagReturned;
    }
    ...
    {
    	return  " Width = " + simage.getWidth() + ", Height = " + simage.getHeight() + 
    ...
    public static byte[] dumpAllTags(int tag, String inFile, int[]error)
    {
    	...
    	{  		
    		...
    		for(pageIndex = 0; pageIndex < totalPage; pageIndex++)
    		{
    			...
    			{		
    				tagNames = saveTags(getTagDescription(tag), "");
    				tagValue = returnedTagVal(tag);
    				byte[] strBuff = stringBuff(count);
    				strTempString  = new String(strBuff, "UTF-16LE");
    				//strTag = saveTags(getTagDescription(tag),strTempString);
    				
    				stat = simage.IMGLOW_get_tiff_tag(tag, maxBytes,tagValue,inFile,strBuff,pageIndex);
...
//        			strItem = "   " + " TagValue = " +  tag + ", \"" + tagNames.toString() + "\", value = " + tagValue.length + "\"  PageNumber = " +  totalPageCount +  "\n";
...
//    				System.out.println("----------------------------------------------------------------------");
        			...
        			{		
        				...
        				//tagNamesBuff = strBuff;
        				


        				strItem += " strTag "  +  "[TagValue] = " +  iTagNum + ", \"" + tagNames + "\", value = " + iTagValue.length + "\"  PageNumber = " +  totalPageCount +  "\n";
    ...
    public static void main(String[]args) throws IOException
    {
    	...
    	TiffImageTags tiffImage = new TiffImageTags(console);
    	int tag = 0;
    	int[] error = new int[1];
    	error[0] = 0;
    	
    	String afile = ("C:\\temp\\DaytonaBikeWeek064.tif");
    	if(new File(afile).exists() == true)
    	{
    		dumpAllTags(tag, afile, error);
    		accessFile = false;
