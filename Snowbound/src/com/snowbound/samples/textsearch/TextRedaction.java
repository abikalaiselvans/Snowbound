...
public class TextRedaction extends Frame
{
    private static final long serialVersionUID = -2257537940727604263L;
    static TextRedaction mFrame = null;
    ...
    public void searchAndDisplay()
    {
        ...
        try
        {
            ...
            createHighlightRects(mImagePanel.getGraphics(), false);
        }
        catch (Throwable t)
        {
            t.printStackTrace();
     ...
     **************************************************************************/
    public void createHighlightRects(Graphics aGraphics, boolean isRedaction)
    {
    	if  (isRedaction == false)
    		annLayer.SANN_set_fcolor(Color.yellow.getRed(),
                                     Color.yellow.getGreen(),
                                     Color.yellow.getBlue());
    	else
    		annLayer.SANN_set_fcolor(Color.black.getRed(),
                                     Color.black.getGreen(),
                                     Color.black.getBlue());

        if (mSearchResults != null)
        {
        	Rectangle2D.Double[] redactionRects = null;
            ...
            for (int nIx = 0; nIx < mSearchResults.length; nIx++)
            {
                ...
                for (int r = 0; r < recta.length; r++)
                {
                    ...
                    Rectangle2D.Double rect = getRectangleFromSannRect(sannRect,
					                    		mSnowbound.getWidth(),
					                    		mSnowbound.getHeight(),
					                            mSnowbound.getXdpi());
                    vRects.add(rect);
                    if  (isRedaction == false)
                    	...
                    	annLayer.SANN_add_object(SnowAnn.SANN_FILLED_RECT,
                                sannRect,
                                null,
                                null,
                                0);
            ...
            if (isRedaction == true)
            {
            	redactionRects = new Rectangle2D.Double[vRects.size()];
                for (int rectIndex = 0; rectIndex < redactionRects.length; rectIndex++)
                {
                    redactionRects[rectIndex] = (Rectangle2D.Double) vRects.elementAt(rectIndex);
                }
            
                int status = mSnowbound.IMGLOW_redact_page(mFileName,
            		mFileName+".Redacted.pdf",
                    redactionRects,
                    mPage,
                    0);
             }	
        ...
        try
        {
        	if (nStat < 0)
            {
                ...
                System.out.println("Caught error in get pages for file ".toString() + 
                		           mCurrentFile + "': " + nStat + " not allowed ".toUpperCase() +
                		           "\n");
            }
        	else{
    ...
    public void layoutSearchPanel()
    {
        ...
        mSearchPanel.setBounds(0, 1, mFrame.getWidth(), nSearchHeight);
        ...
        mSearch.setBounds(mTextField.getX() + mTextField.getWidth() + 5,
                          mTextField.getY(), 55, 23);
        
        mRedact = new Button("Redact");
        mRedact.setBounds(mSearch.getX() + mSearch.getWidth() + 10,
                          mSearch.getY(), 75, 23);
        
        mZoomIn = new Button("Zoom In");
        mZoomIn.setBounds(mRedact.getX() + mRedact.getWidth() + 50,
        		          mRedact.getY(), 75, 23);
        ...
        mSearchPanel.add(mSearch);
        mSearchPanel.add(mRedact);
        ...
        mRedact.setEnabled(false);
        mRedact.addActionListener(new ActionListener()
        {
            public void actionPerformed(ActionEvent e)
            {
                createHighlightRects(mImagePanel.getGraphics(), true);
    ...
    {
        mFrame = new TextRedaction();
        mFrame.execute();
    }
    
    private Rectangle2D.Double getRectangleFromSannRect(SANN_RECT rc,
                                                        int imageWidth,
                                                        int imageHeight,
                                                        int dpi)
    {
		float pixelsToPts = (72f / dpi);
		
		int rectX = (int) (rc.left * pixelsToPts);
		int rectWidth = (int) ((rc.right - rc.left) * pixelsToPts);
		int rectHeight = (int) ((rc.bottom - rc.top) * pixelsToPts);
		
		/* Important FIXME - if / when the library is converted to use
         * traditional coordinate space, rect Y should be assigned as in
         * the commented line below. Until then, we need to consider the
         * imageHeight and subtract.         
		 */
		 //        int rectY = (int) (rc.top * pixelsToPts);
		
		int rectY = (int) (imageHeight * pixelsToPts) -
			        (int) (rc.bottom * pixelsToPts);
